...
// Fireworks Variables
vec3 p_velocity[NUM_PARTICLES];
vec3 p_color[NUM_PARTICLES];
float t_start = 0;
float t = 0;
...
float getTime(){
    float t_sub = (float)glutGet(GLUT_ELAPSED_TIME);
    float tDiff = t - t_sub; 
    // std::cout << tDiff << std::endl;
    return findMod(tDiff, T_MAX);
}
void setFirework(){
...
void init(){
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(p_velocity) + sizeof(p_color), NULL, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(p_velocity), p_velocity);
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(p_velocity), sizeof(p_color), p_color);
...
void drawFirework(){
	...
	glVertexAttribPointer(vColor, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(p_velocity))); 
...
// void drawFireworks(GLuint buffer, GLuint drawMode, GLuint firework_program){
//     //--- Activate the vertex buffer object to be drawn ---//
//     glBindBuffer(GL_ARRAY_BUFFER, buffer);
...
void display() {
    ...
    /*----- Draw Fireworks -----*/  
    // mv = LookAt(eye, at, up);   // Set mv to center
    // model_view = glGetUniformLocation(fireworks_program, "model_view" );
    // projection = glGetUniformLocation(fireworks_program, "projection" );
    // float temp = (float)glutGet(GLUT_ELAPSED_TIME) * 0.001;
    // std::cout << temp << std::endl;
    // glUniform1f(glGetUniformLocation(fireworks_program, "t"), temp);
    // drawFirework();
}
//---------------------------------------------------------------------------
